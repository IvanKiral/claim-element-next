...
export const GET = async (request: NextRequest) => {
  ...
  if (!itemCodename) {
    return Response.json({ error: "itemCodenmae is required" }, { status: 400 });
  }

  if (!languageCodename) {
    return Response.json({ error: "languageCodename is required" }, { status: 400 });
  }
  ...
  const workflow = await client
    .listWorkflows()
    .toPromise()
    .then((res) => res.data.find((w) => w.id === variant.workflow.workflowIdentifier.id));
  ...
  return Response.json({
    ...
    workflow: {
      ...workflow,
      step_identifier: {
        ...workflow.steps.find((s) => s.id === variant.workflow.stepIdentifier.id),
      },
    },
  });
};
