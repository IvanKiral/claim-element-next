...
import type { GetItemResponse } from "./shared/types/routes.ts";
...
const CustomElement = () => {
  ...
  const data = useQuery({
    ...
    queryFn: () =>
      fetch(
        `/api/get-item?environmentId=${environmentId}&itemCodename=${item.codename}&languageCodename=${variant.codename}`,
  ...
  if (
    data.data?.contributors.some((u) => u.id === userContext?.id) &&
    data.data?.workflow.step_identifier.codename === config.assignedStepCodename
  ) {
    return (
      <div className={`container py-4 flex flex-col w-full items-center space-y-4`}>
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md w-full text-center shadow-sm">
          ...
          <p className="text-blue-700 text-s leading-relaxed">
            If you were recently assigned to this item, <strong>refresh your browser</strong> to
            start editing.
          </p>
        </div>
        <main>
          <button
            ...
            onClick={() =>
              unassignItemMutation.mutate({
                ...
                languageCodename: variant.codename,
              })
            }
    ...
    <div className={`container py-4 flex flex-col w-full items-center space-y-4`}>
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md w-full text-center shadow-sm">
        ...
        <p className="text-blue-700 text-xs leading-relaxed">
          This item is currently unassigned to an editor. Please click &quot;Assign to Me&quot; to
          lock it for editing.
        </p>
      </div>
      <main>
